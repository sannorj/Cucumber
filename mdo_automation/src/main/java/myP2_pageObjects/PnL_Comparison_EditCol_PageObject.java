...
public class PnL_Comparison_EditCol_PageObject {
	...
	public void addColumnFunc() throws InterruptedException {
		...
		WebElement drpColumn3Ele = new WebDriverWait(driver, Duration.ofSeconds(20))
				.until(ExpectedConditions.visibilityOf(drpColumn3));
		drpColumn3Ele.click();
		Thread.sleep(10000);
		for (int i = 0; i < listDrpThree.size(); i++) {
			if (listDrpThree.get(i).getText().equalsIgnoreCase(configReader.getProp("PnLE_Column"))) {
				listDrpThree.get(i).click();
				Thread.sleep(5000);
			}
		}
		...
		try {
			for (int i = 0; i < listDrpThree.size(); i++) {
				if (listDrpThree.get(i).getText().equalsIgnoreCase(configReader.getProp("PnLE_Year"))) {
					listDrpThree.get(i).click();
				}
			}
	...
	public boolean verifyNewlyAddedColFunc() throws InterruptedException {
		...
		if (headerName.equalsIgnoreCase(configReader.getProp("PnLE_Column"))
				&& headerYear.equalsIgnoreCase(configReader.getProp("PnLE_Year"))) {
			flag = true;
		} else {
			flag = false;
		}
	...
	public boolean verifyNameList() {
		...
		for (int i = 0; i < PnLENameList.size(); i++) {
			...
			int rawVisible = driver
					.findElements(By.xpath("//table/tbody/tr/td[1]//div[text()='" + PnLENameList.get(i) + "']")).size();
			if (rawVisible < 1)
				rawExist = false;
		}
		return rawExist;
