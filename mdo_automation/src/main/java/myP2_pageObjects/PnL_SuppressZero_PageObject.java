...
public class PnL_SuppressZero_PageObject {
	...
	public void selectParameters() throws InterruptedException {
		...
		try {
			for (int i = 0; i < lstDrpView.size(); i++) {
				if (lstDrpView.get(i).getText().equalsIgnoreCase(configReader.getProp("SZ_View"))) {
					ExpectedConditions.visibilityOf(lstDrpView.get(0));
					lstDrpView.get(i).click();
				}
			}
