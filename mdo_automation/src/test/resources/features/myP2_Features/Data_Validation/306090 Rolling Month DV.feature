...
Feature: Report 306090 data validation Functionality 
 ...
 #Author: sannorjP@mydigitaloffice.ca
   Scenario Outline: Verifying whether the Property values in all the columns are matching with the respective column totals when summed
    Given I am login to the myp2 site
    And System navigate to the home page
    And Select the organization as "HighGate Hotels"
    And Click on the Menu bar
    And Expand the '<Main_menu>' menu
    Then Expand the '<sub_menu>' menu option
    And Select the '<target_Page>' option
    Given Check the header of the navigated page '<target_Page_header>'
    When User selects the Group and date
    And verify whether calender month total are matching with the total row value    
    
      Examples: 
      	|  Main_menu  | sub_menu  |     target_Page        |  	    target_Page_header     | 
      	|  	Reports 	|  Revenue  |  30/60/90 Report (New) | 30/60/90 Rolling Month Report | 
